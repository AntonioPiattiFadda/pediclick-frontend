      ...
      <div className="flex items-center justify-between px-4 md:px-6 py-4">
        ...
        <div className="flex items-center gap-3">
          <div className="hidden md:block text-right">
            ...
            <p className="text-xs text-muted-foreground">Rol: {ROLES.find(role => role.value === userData?.role)?.label}</p>
          </div>

          <Avatar className="w-8 h-8">
            <AvatarImage src={`https://ui-avatars.com/api/?name=${userData?.full_name || userData?.email}`} alt="Usuario" />
        ...
        </div> :  <Skeleton className="h-8 w-48"/>}
